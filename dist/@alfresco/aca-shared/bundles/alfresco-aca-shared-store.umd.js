(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@angular/material/dialog'), require('@angular/router'), require('@alfresco/adf-core'), require('@angular/material/snack-bar'), require('rxjs/operators'), require('@ngrx/store'), require('@angular/core'), require('@ngrx/effects')) :
    typeof define === 'function' && define.amd ? define('@alfresco/aca-shared/store', ['exports', '@angular/material/dialog', '@angular/router', '@alfresco/adf-core', '@angular/material/snack-bar', 'rxjs/operators', '@ngrx/store', '@angular/core', '@ngrx/effects'], factory) :
    (factory((global.alfresco = global.alfresco || {}, global.alfresco['aca-shared'] = global.alfresco['aca-shared'] || {}, global.alfresco['aca-shared'].store = {}),global.ng.material.dialog,global.ng.router,global.adfCore,global.ng.material['snack-bar'],global.rxjs.operators,global.store,global.ng.core,global.effects));
}(this, (function (exports,dialog,router,adfCore,snackBar,operators,store,core,effects) { 'use strict';
    ...
    ***************************************************************************** */
    ...
    var DialogEffects = /** @class */ (function () {
        function DialogEffects(actions$, matDialog) {
            ...
            this.closeAll$ = this.actions$.pipe(effects.ofType(AppActionTypes.CloseModalDialogs), operators.map(function () { return _this.matDialog.closeAll(); }));
        }
    ...
    var RouterEffects = /** @class */ (function () {
        function RouterEffects(store$$1, actions$, router$$1) {
            ...
            this.navigateUrl$ = this.actions$.pipe(effects.ofType(RouterActionTypes.NavigateUrl), operators.map(function (action) {
                if (action.payload) {
                    _this.router.navigateByUrl(action.payload);
                }
            }));
            this.navigateRoute$ = this.actions$.pipe(effects.ofType(RouterActionTypes.NavigateRoute), operators.map(function (action) {
                _this.router.navigate(action.payload);
            }));
            this.navigateToFolder$ = this.actions$.pipe(effects.ofType(RouterActionTypes.NavigateFolder), operators.map(function (action) {
                if (action.payload && action.payload.entry) {
                    _this.navigateToFolder(action.payload.entry);
                }
            }));
            this.navigateToParentFolder$ = this.actions$.pipe(effects.ofType(RouterActionTypes.NavigateParentFolder), operators.map(function (action) {
                if (action.payload && action.payload.entry) {
    ...
    var SnackbarEffects = /** @class */ (function () {
        function SnackbarEffects(store$$1, actions$, snackBar$$1, translationService) {
            var _this = this;
            this.store = store$$1;
            this.actions$ = actions$;
            this.snackBar = snackBar$$1;
            this.translationService = translationService;
            this.infoEffect = this.actions$.pipe(effects.ofType(SnackbarActionTypes.Info), operators.map(function (action) {
                _this.showSnackBar(action, 'info-snackbar');
            }));
            this.warningEffect = this.actions$.pipe(effects.ofType(SnackbarActionTypes.Warning), operators.map(function (action) {
                _this.showSnackBar(action, 'warning-snackbar');
            }));
            this.errorEffect = this.actions$.pipe(effects.ofType(SnackbarActionTypes.Error), operators.map(function (action) {
    ...
    /** @type {?} */
    var getHeaderColor = store.createSelector(selectApp, function (state) { return state.headerColor; });
    /** @type {?} */
    var getAppName = store.createSelector(selectApp, function (state) { return state.appName; });
    /** @type {?} */
    var getLogoPath = store.createSelector(selectApp, function (state) { return state.logoPath; });
    /** @type {?} */
    var getLanguagePickerState = store.createSelector(selectApp, function (state) { return state.languagePicker; });
    /** @type {?} */
    var getUserProfile = store.createSelector(selectApp, function (state) { return state.user; });
    /** @type {?} */
    var getCurrentFolder = store.createSelector(selectApp, function (state) { return state.navigation.currentFolder; });
    /** @type {?} */
    var getAppSelection = store.createSelector(selectApp, function (state) { return state.selection; });
    /** @type {?} */
    var getSharedUrl = store.createSelector(selectApp, function (state) { return state.sharedUrl; });
    /** @type {?} */
    var getNavigationState = store.createSelector(selectApp, function (state) { return state.navigation; });
    /** @type {?} */
    var isInfoDrawerOpened = store.createSelector(selectApp, function (state) { return state.infoDrawerOpened; });
    /** @type {?} */
    var showFacetFilter = store.createSelector(selectApp, function (state) { return state.showFacetFilter; });
    /** @type {?} */
    var getDocumentDisplayMode = store.createSelector(selectApp, function (state) { return state.documentDisplayMode; });
    /** @type {?} */
    var getRepositoryStatus = store.createSelector(selectApp, function (state) { return state.repository; });
    /** @type {?} */
    var isQuickShareEnabled = store.createSelector(getRepositoryStatus, function (info) { return info.status.isQuickShareEnabled; });
    /** @type {?} */
    var isAdmin = store.createSelector(selectApp, function (state) { return state.user.isAdmin; });
    /** @type {?} */
    var getSideNavState = store.createSelector(getAppSelection, getNavigationState, function (selection, navigation) {
        return {
            selection: selection,
    ...
    /** @type {?} */
    var getRuleContext = store.createSelector(getAppSelection, getNavigationState, getUserProfile, getRepositoryStatus, function (selection, navigation, profile, repository) {
        return {
            selection: selection,
            navigation: navigation,
            profile: profile,
    ...
    /** @type {?} */
    var infoDrawerMetadataAspect = store.createSelector(selectApp, function (state) { return state.infoDrawerMetadataAspect; });
    ...
    var SharedStoreModule = /** @class */ (function () {
        ...
        SharedStoreModule.decorators = [
            { type: core.NgModule, args: [{
                        imports: [
                            effects.EffectsModule.forFeature([SnackbarEffects, DialogEffects, RouterEffects])
                        ]
                    },] }
        ];
    ...
    exports.SetSettingsParameterAction = SetSettingsParameterAction;
    exports.SetInitialStateAction = SetInitialStateAction;
    exports.SetHeaderColorAction = SetHeaderColorAction;
